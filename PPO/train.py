...
class Actor(nn.Module):
    ...
    def compute_proba(self, state, action):
        ...
        mu, sigma = self.model(state), torch.exp(self.sigma)
        p = Normal(mu, sigma)

        return torch.exp(p.log_prob(action).sum(-1)), p

    def act(self, state):
        # Returns an action (with tanh), not-transformed action (without tanh) and distribution of non-transformed actions
        # Remember: agent is not deterministic, sample actions from distribution (e.g. Gaussian)
        distribution = Normal(self.model(state), torch.exp(self.sigma))
...
class PPO:
    def __init__(self, state_dim, action_dim):
        self.actor = torch.load("agent.pkl").to(device) #Actor(state_dim, action_dim).to(device)  #torch.load("agent.pkl").to(device)
        self.critic = torch.load("criric.pkl").to(device) #Critic(state_dim).to(device)  # torch.load("criric.pkl").to(device)
    ...
    def update(self, trajectories):
        ...
        for _ in range(BATCHES_PER_UPDATE):
            ...
            op = torch.tensor(
                old_prob[idx]
            ).float().to(device)  # Probability of the action in state s.t. old policy
            v = torch.tensor(target_value[idx]).float().to(device)  # Estimated by lambda-returns
            adv = torch.tensor(
                advantage[idx]
            ).float().to(device)  # Estimated by generalized advantage estimation

            self.actor_optim.zero_grad()
            self.critic_optim.zero_grad()

            proba, distribution = self.actor.compute_proba(s, a)
            ratio = torch.exp(torch.log(proba + 1e-6) - torch.log(op + 1e-6))
            loss1 = ratio * adv
            loss2 = torch.clamp(ratio, 1 - CLIP, 1 + CLIP) * adv
            loss_actor = (-torch.min(loss1, loss2)).mean() - ENTROPY_COEF * distribution.entropy().mean()
            loss_actor.backward()
            self.actor_optim.step()

            loss_critic = F.smooth_l1_loss(self.critic.get_value(s).view(-1), v)
    ...
    def save(self):
        ...
        torch.save(self.critic, "criric.pkl")
...
if __name__ == "__main__":
    ...
    print("act space ", env.action_space.shape[0])
    ppo = PPO(
